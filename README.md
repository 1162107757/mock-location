...
- 首页基础固定位置模拟无需卡密；轨迹、自绘、导航和收藏功能支持离线设备绑定卡密
...
应用不再提供自动试用。固定位置、当前位置、地图选点、历史位置、Root 配置、轨迹、自绘、导航和收藏等功能都需要有效卡密；“卡密与授权”页面始终可以打开用于激活。授权方可以使用 `tools/license/LicenseKeyTool.java` 或 `Mock License Issuer` APK 手动生成限时试用卡，按天数设置有效期，也可以生成永久卡。点击“卡密与授权”可查看设备码和 `MLK2` 设备密钥指纹，并复制给授权方。新卡密使用 `ML2` 格式：由发卡私钥签名，绑定设备密钥、公包名和签名证书；App 通过内置公钥在本机验证签名、有效期和功能范围，再使用 Android Keystore 加密保存授权状态。旧版 `ML1` 卡密仍可用于迁移，但建议重新签发 ML2。该流程不需要服务端；无服务端模式不能远程撤销已经发出的卡密，也不能防止修改 APK 绕过授权。
